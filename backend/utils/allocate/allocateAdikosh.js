...
module.exports = async function allocateEV(lan, payment) {
  ...
  while (remaining > 0) {
    const [emi] = await queryDB(
      `SELECT * FROM manual_rps_adikosh WHERE lan = ? AND (remaining_interest > 0 OR remaining_principal > 0) ORDER BY due_date ASC LIMIT 1`,
      [lan]
    );
    if (!emi) break;
    ...
    await queryDB(
      `UPDATE manual_rps_adikosh SET remaining_interest=?, remaining_principal=?, remaining_emi=?, remaining_amount=?, payment_date=? WHERE id=?`,
